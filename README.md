...
Every design decision in SecureVote is governed by five non-negotiable principles, listed in priority order:
...
SecureVote operates across four layers:
...
To ensure the ID requirement does not disenfranchise eligible voters, SecureVote assumes a companion policy:
...
To preserve ballot secrecy while still preventing double-voting, SecureVote uses a **blind signature scheme**:
